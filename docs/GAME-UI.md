...
- 连接检查超时为 20 秒，设计后台进程硬上限为 35 分钟。Hub 单次请求默认 300 秒、最多 600 秒；全部生成/修正共享最多三次调用，每种恢复策略最多一次。可在 AI Hub 的生成设置中调整。
- 失败任务的诊断路径显示在面板中，记录保留最近 32 份；通过 Hub 请求编号定位具体模型、参数、终止原因及恢复请求。
